...
int main(int argc, char *argv[]){
	...
	cout<<"Client ip:"<<inet_ntoa(clientaddr.sin_addr)<<"\tClient port:"<<ntohs(clientaddr.sin_port)<<endl;
	
	
	//5.data transfer
	char rec[1024];
	char mark[]={"your input:"};
	int iconnect = 0;
	while(1){
		memset(rec,0,sizeof(rec));
		int set = read(conn,rec,sizeof(rec));//set 接收字符串的长度
		cout<<rec<<endl;
		//write(conn,mark,sizeof(mark));
		//strcat(mark,rec);		
		write(conn,rec,set);
		iconnect++;
		if(iconnect>=5)break;
